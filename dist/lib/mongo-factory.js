...
class MongoDatabase {
    constructor(ctx, conn) {
        this.db = conn || _mongoose2.default;
        this.ctx = ctx;
    }
    ...
    _ensureModel(model) {
        return (0, _utils.isString)(model) ? this.db.model(model) : model;
    }
...
const DBManager = exports.DBManager = {
    dbs: {},
    models: {},
    length: 0,
    init: ctx => {
    ...
    connectDB: (db_name = DEFAULT_DB_NAME, options) => {
        ...
        if (!db) {
            ...
            let conn = _mongoose2.default.createConnection(uri, { useMongoClient: true });
    ...
    loadModels: async (dir, db_name = DEFAULT_DB_NAME) => {
        let schemaFiles = await (0, _utils.readDirectoryStructure)(dir, { format: 'array', exts: '.js' });
