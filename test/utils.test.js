...
describe('module [utils]', () => {
  ...
  let str = 'this is a string', number = 3, arrRange, monthRange, dateRange, yearRange,
    ...
    objPluck = [{a: 1, b: 2, c: 3, d: 4, e: 5}, {a: 11, b: 12, c: 13, d: 14, e: 15}],
    objPluckStr = JSON.stringify(objPluck),
    func1 = () => {
    }, func1Str = func1.toString(), func2 = () => 1, func2Str = func2.toString(),
    ...
    arr = ['aa', 134, () => {
      return 2;
    }], arrStr = JSON.stringify(arr), arrChunked,
    objUnflatten, objUnflattenStr, objFlatten, objFlattenStr,
    thunkify_executed_value = 3, fn_to_thunk = (num1, num2, callback) => {
      setTimeout(() => {
        callback(null, num1 + num2)
      }, 1000)
    }, fn_to_thunk_str = fn_to_thunk.toString();
  ...
  describe(`test function <omit>`, () => {
    it(`let objPick = ${objPickStr}, omit(objPick, 'a','e') expect to be {b:2, c:3, d:4}`, () => {
      let retOmit = omit(objPick, 'a', 'e');
      expect(Object.keys(retOmit).length).to.be.equal(3);
      expect(retOmit['b']).to.be.equal(2);
      expect(retOmit['c']).to.be.equal(3);
      expect(retOmit['d']).to.be.equal(4);
    });
  ...
  describe(`test function <readDirectoryStructure>`, () => {
    it("readDirectoryStructure to.be.not.empty", function (done) {
      readDirectoryStructure(__dirname + '/../', {
        format: 'tree',
        exts: ['.js', '.json'],
